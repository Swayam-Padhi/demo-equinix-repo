...
parser.add_argument("--entry_type", required=True, choices=["asset", "table", "column"], help="Entry type: asset, table, or column")
...
parser.add_argument("--table", required=False, help="Table name (for table or column entry types)")
...
parser.add_argument("--include_columns", required=False, default="false", choices=["true", "false"],
                    help="For table entry_type, attach aspects to all columns too")
args = parser.parse_args()

INCLUDE_COLUMNS = args.include_columns.lower() == "true"
    ...
    for zone in zones:
        ...
        for asset in assets:
            ...
            if args.entry_type == "asset":
                ...
                if attach_aspects(headers, entry_name, aspects_json):
                    success_count += 1
            ...
            # ----------------- Table Level -----------------
                ...
                for table in tables:
                    ...
                    if args.entry_type == "table":
                        ...
                        if attach_aspects(headers, table_entry_name, aspects_json):
                            success_count += 1
                        ...
                        if INCLUDE_COLUMNS:
                            ...
                            for column in schema:
                                column_entry_id = f"{table_entry_id}/columns/{column.name}"
                                column_entry_name = f"projects/{PROJECT_ID}/locations/{LOCATION}/entryGroups/{ENTRY_GROUP}/entries/{quote(column_entry_id)}"
                                print(f" Attaching aspects to column: {column_entry_name}")
                                if attach_aspects(headers, column_entry_name, aspects_json):
                                    success_count += 1

                    # Column-level attach (one specific column only)
                    elif args.entry_type == "column":
                        if not args.column:
                            print("❌ Column name required for entry_type=column.")
                            sys.exit(1)
                        column_entry_id = f"{table_entry_id}/columns/{args.column}"
                        column_entry_name = f"projects/{PROJECT_ID}/locations/{LOCATION}/entryGroups/{ENTRY_GROUP}/entries/{quote(column_entry_id)}"
                        print(f" Attaching aspects to specific column: {column_entry_name}")
                        if attach_aspects(headers, column_entry_name, aspects_json):
                            success_count += 1

    if success_count == 0:
        print("⚠️ No aspects were attached successfully.")
        sys.exit(1)
    else:
